...
Page({
  ...
  operation(e) {//录制直播开启和关闭
    ...
    util.sendSocketMessage({
      data: obj, that: this, success: function () {
        // console.log('success');
        wx.showToast({
          title: '操作成功',
          icon: 'none',
  ...
  live(e) {//开关直播
    ...
    util.sendSocketMessage({
      data: msg, that: this, success: function () {
        // console.log('success');
        wx.showToast({
          title: '操作成功',
          icon: 'none',
    ...
    util.sendSocketMessage({
      data: msg1, that: this, success: function () {
        // console.log('success');
        wx.showToast({
          title: '操作成功',
          icon: 'none',
  ...
  liveCallBack(res) {//处理直播返回
    console.log(res.indexOf('Success') == -1);
    if (res.indexOf('Connect Server Failed') != -1) {
      wx.showToast({
        title: '直播开启失败，请设置直播推流地址',
        icon: "none",
        duration: 3000
      });
      return;
